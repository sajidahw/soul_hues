...
export const userSignup = async (
  ...
  try {
    ...
    if (existingUser)
      return res.status(401).send("Whoops, User has already been registered.");
    ...
    return res
      .status(201)
      .json({ message: "OK", name: user.name, email: user.email }); // response for status code 200 to all users, can see user or userid converted to string from json; removed id: user._id.toString()
  } catch (error) {
    console.log(error);
    return res.status(400).json({ message: "ERROR", cause: error.message });
  }
